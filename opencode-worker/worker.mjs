...
const WORKSPACE_TOOLS = ["read", "glob", "grep", "bash", "write", "edit"]
...
const PROOF_RECEIPT_FILENAME = ".apkscanner-proof-receipts.jsonl"
...
async function investigate(client, payload) {
  ...
  const pocInventory = analysisStage?.workspace_tools
    ? await workspacePocInventory(process.cwd())
    : undefined
  const proofReceipts = await verifiedProofReceipts(process.cwd())
  if (proofReceipts.length > 0) {
    payload.platform_proof_receipts = proofReceipts
    payload.allowed_evidence_ids = [
      ...new Set([
        ...(payload.allowed_evidence_ids ?? []),
        ...proofReceipts.flatMap((receipt) => receipt.evidence_ids),
      ]),
    ]
    payload.protected_hypothesis_ids = [
      ...new Set([
        ...(payload.protected_hypothesis_ids ?? []),
        ...proofReceipts
          .filter((receipt) => receipt.result === "reproduced_blackbox")
          .map((receipt) => receipt.hypothesis_id),
      ]),
    ]
    emitRuntimeEvent(
      "platform.proof.receipts.loaded",
      "已装载并验证本轮平台动态证明回执",
      {
        receipt_count: proofReceipts.length,
        evidence_count: payload.allowed_evidence_ids.length,
        protected_hypothesis_count: payload.protected_hypothesis_ids.length,
      },
    )
  }
  const promptText = explorerMemo
    ? finalizerPrompt(payload.prompt, explorerMemo, pocInventory, proofReceipts)
    : appendProofReceipts(payload.prompt, proofReceipts)
...
async function runStructuredStage(
  ...
  for (let index = 0; index <= maxRetries; index += 1) {
    ...
    if (parsedObject && schemaValid) {
      applyAuthoritativeProofReceipts(parsed.value, payload)
    }
...
function applyAuthoritativeProofReceipts(value, payload) {
  const receipts = (payload.platform_proof_receipts ?? []).filter(
    (receipt) => receipt.result === "reproduced_blackbox",
  )
  ...
  for (const receipt of receipts) {
    evidenceByHypothesis.set(receipt.hypothesis_id, [
      ...new Set([
        ...(evidenceByHypothesis.get(receipt.hypothesis_id) ?? []),
  ...
  value.evidence_ids = [
    ...new Set([
      ...
      ...receipts.flatMap((receipt) => receipt.evidence_ids),
    ]),
  ]
  if (!Array.isArray(value.hypothesis_assessments)) return
  for (const assessment of value.hypothesis_assessments) {
    if (!assessment || typeof assessment !== "object" || Array.isArray(assessment)) {
      continue
    }
    const platformEvidence = evidenceByHypothesis.get(assessment.hypothesis_id)
    if (!platformEvidence) continue
    assessment.verdict = "reproduced_blackbox"
...
function finalizerPrompt(basePrompt, explorerMemo, pocInventory, proofReceipts) {
  const pocInventoryPrompt = pocInventory
    ? (
  ...
  return (
    ...
    pocInventoryPrompt +
    proofReceiptPrompt(proofReceipts)
  )
}

function appendProofReceipts(basePrompt, proofReceipts) {
  if (proofReceipts.length === 0) return basePrompt
  return `${basePrompt}${proofReceiptPrompt(proofReceipts)}`
}

function proofReceiptPrompt(proofReceipts) {
  if (proofReceipts.length === 0) return ""
  return (
    "\n\nPLATFORM_LIVE_PROOF_RECEIPTS:\n" +
    "These receipts were HMAC-verified by the worker after the workspace analyzer completed. " +
    "They are authoritative platform evidence from this exact task and session. Preserve each " +
    "receipt's hypothesis_id, result, and Evidence IDs exactly. A reproduced_blackbox receipt " +
    "overrides conflicting static reasoning for that hypothesis.\n\n" +
    `${JSON.stringify(proofReceipts, null, 2)}`
  )
}

async function verifiedProofReceipts(root) {
  const token = process.env.APKSCANNER_PROOF_TOKEN
  if (typeof token !== "string" || token.length < 16) return []
  let contents
  try {
    contents = await readFile(join(root, PROOF_RECEIPT_FILENAME), "utf8")
  } catch {
    return []
  }
  if (Buffer.byteLength(contents, "utf8") > 2 * 1024 * 1024) {
    throw new Error("platform proof receipt file exceeds the worker safety limit")
  }
  const receipts = []
  for (const line of contents.split(/\r?\n/)) {
    ...
    if (
      typeof unsigned.hypothesis_id !== "string" ||
      !/^[a-f0-9-]{36}$/.test(unsigned.hypothesis_id) ||
      !["reproduced_blackbox", "inconclusive"].includes(unsigned.result) ||
      !Array.isArray(unsigned.evidence_ids) ||
      unsigned.evidence_ids.some(
        (item) => typeof item !== "string" || !/^[a-f0-9-]{36}$/.test(item),
      )
    ) {
      continue
    }
    receipts.push(unsigned)
  }
  return strongestProofReceipts(receipts)
}

function strongestProofReceipts(receipts) {
  const selected = new Map()
  for (const receipt of receipts) {
    const current = selected.get(receipt.hypothesis_id)
    if (
      current?.result === "reproduced_blackbox" &&
      receipt.result !== "reproduced_blackbox"
    ) {
      continue
    }
    selected.set(receipt.hypothesis_id, receipt)
  }
