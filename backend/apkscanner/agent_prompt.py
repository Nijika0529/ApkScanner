    ...
    *,
    ...
    if direct_tool_access and shell_access and workspace_write:
        tool_boundary = (
            ...
            "source-only Android PoC under poc/<name>/. When live proof replay is available, "
            "submit the final working project with apkscanner-proof instead of requested_tests."
        )
    ...
    exploration_discipline = (
        ...
        "when impact requires an app UID, Binder client, returned Provider data, or WebView result "
        "capture, move directly to one dedicated PoC instead of searching logcat for an observation "
        "the target does not emit. After creating and verifying that PoC, submit exactly one live "
        "proof replay rather than continuing raw ADB exploration. "
        "If two ordinary-app PoC executions have already answered the same hypothesis, do not "
        "fall back to repeated adb-shell broadcasts, starts, force-stops, dumpsys filters, or "
        "logcat variants: shell identity cannot upgrade that proof. Either change one concrete "
        "control variable or Oracle predicate in the next PoC, or stop dynamic exploration and "
    ...
    *,
    ...
    if direct_tool_access and shell_access and workspace_write:
        access_instruction = (
            ...
            "builder compile it. Alternatively "
            ...
            "every referenced file with realpath and test -f before invoking proof. Its "
    ...
    return (
        f"{phase_instruction} Treat the assigned Android entry point as the mandatory root and "
        "scope boundary of this task. The complete APK workspace is available only so an exact "
        "edge discovered from that root can be followed across "
        "helper classes, callbacks, non-exported components, Binder/AIDL, Providers, WebViews, files, "
        "databases, or native boundaries. Do not inventory, grep for, or open unrelated components "
        ...
        "The proof JSON contains poc, oracle, rationale, and optional extras/reset; hypothesis_id "
        "and entry_point_id are optional because the platform associates the current task defaults. "
        "Use this shape: {\"poc\":{\"project_path\":\"poc/name\",\"package_name\":"
        "\"io.apkscanner.poc.name\",\"launch_component\":\".MainActivity\",\"log_tag\":"
        "\"APKSCANNER_POC\"},\"oracle\":{\"kind\":\"log_contains\",\"expected_text\":"
        "\"security_impact_observed\",\"impact\":\"unauthorized_state_change\"},\"rationale\":"
        ...
        "build a dedicated ordinary-app PoC and submit it through the live proof command. "
        ...
        "unauthorized_data_access. A log_contains Oracle attached to a structured PoC may use "
        "unauthorized_data_access, unauthorized_state_change, or privileged_action only when its "
        "expected_text is the concrete observed effect and the PoC reports the correlated structured "
        "impact result; otherwise use impact=none. ui_text may use unauthorized_data_access only and becomes harm "
