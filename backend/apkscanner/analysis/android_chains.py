...
from ..core.enums import EntryPointKind, Severity
...
MARKERS = (
    ...
    MarkerPattern(
        ...
        _patterns(
            r"(?s)new\s+Intent\s*\(.{0,240}(?:\.(?:class|javaClass)|::\s*class(?:\.java)?)"
    ...
    MarkerPattern(
        ...
        _patterns(
            r"new\s+Intent\s*\([^\n;]{0,180}(?:\.(?:class|javaClass)|::\s*class(?:\.java)?)",
    ...
    MarkerPattern(
        "service_manager_access",
        _patterns(
            r"\bServiceManager\s*\.\s*(?:getService|addService|listServices|checkService)\s*\(",
            r"Landroid/os/ServiceManager;->(?:getService|addService|listServices|checkService)\(",
        ),
    ),
    MarkerPattern(
        "self_reported_identity",
        _patterns(
            r"\b(?:getStringExtra|getExtras)\s*\([^)]*[\"'](?:package|pkg|caller|from_package|source_package)[\"']",
            r"\bgetExtras\s*\(\s*\)\s*\.\s*getString\s*\([^)]*[\"'](?:package|pkg|caller)[\"']",
            flags=re.IGNORECASE,
        ),
    ),
    MarkerPattern(
        "plugin_archive_trust",
        _patterns(
            r"\bgetPackageArchiveInfo\s*\(",
            r"\bPackageManager\s*\.\s*getPackageInfo\s*\([^)]*(?:GET_SIGNATURES|GET_SIGNING_CERTIFICATES)",
            r"Landroid/content/pm/PackageManager;->getPackageArchiveInfo\(",
        ),
    ),
)
...
MARKER_NEEDLES: dict[str, tuple[str, ...]] = {
    ...
    "intent_sanitizer": ("IntentSanitizer", "getComponent", "getPackage", "allow"),
    ...
    "service_manager_access": ("ServiceManager",),
    "self_reported_identity": (
        "getStringExtra",
        "getExtras",
        "package",
...
CHAIN_SPECS = (
    ...
    ChainSpec(
        ...
        sinks=frozenset({"caller_supplied_identity", "self_reported_identity"}),
        risks=frozenset({"caller_identity_authorization", "plugin_archive_trust"}),
        guards=frozenset({"binder_caller_identity_guard"}),
    ),
    ChainSpec(
        family="runtime_ipc_boundary",
        chain_kind="service_manager_identity_bypass",
        title="ServiceManager-style multi-service identity bypass",
        severity=Severity.HIGH.value,
        priority=99,
        sources=frozenset({"binder_entrypoint", "service_manager_access"}),
        sinks=frozenset({"caller_supplied_identity", "self_reported_identity"}),
        risks=frozenset({"caller_identity_authorization", "plugin_archive_trust"}),
        guards=frozenset({"binder_caller_identity_guard"}),
    ),
...
class AndroidAttackChainAnalyzer:
    ...
    def analyze(
        ...
        for spec in CHAIN_SPECS:
            for item in self._chains_for_spec(manifest, nodes, adjacency, spec):
                item.setdefault("search_direction", "forward")
                candidates.append(item)

        forward_fingerprints = {str(item["fingerprint"]): item for item in candidates}
        for spec in CHAIN_SPECS:
            for rc in self._reverse_chains_for_spec(
                manifest, nodes, adjacency, spec
            ):
                fp = str(rc["fingerprint"])
                existing = forward_fingerprints.get(fp)
                if existing:
                    existing["search_direction"] = "bidirectional"
                    existing["priority"] = max(100, int(existing.get("priority") or 0) + 3)
                else:
                    candidates.append(rc)
                    forward_fingerprints[fp] = rc
    ...
    def _reverse_adjacency(adjacency: dict[str, set[str]]) -> dict[str, set[str]]:
        """Reverse the adjacency graph: A→B becomes B→A."""
        rev: dict[str, set[str]] = {name: set() for name in adjacency}
    ...
    def _reverse_chains_for_spec(
        ...
        """Reverse chain search: sink→source, catches paths forward search missed.

        Uses the *original* adjacency graph because the edge direction is
        sink→source (the sink node references the source node).  Forward
        search starts from source nodes and follows edges outward; reverse
        search starts from sink nodes and follows edges outward — the same
        BFS, just with different start nodes.
        """
        source_nodes = {name for name, node in nodes.items() if set(node.markers) & spec.sources}
        sink_nodes = [name for name, node in nodes.items() if set(node.markers) & spec.sinks]
        candidates: list[dict[str, Any]] = []
        for sink_name in sorted(sink_nodes)[:80]:
            sink_node = nodes[sink_name]
            effective_sources = set(source_nodes)
            if sink_name in effective_sources and not self._markers_share_context(
                sink_node, spec.sinks, spec.sources
            ):
                effective_sources.remove(sink_name)
            paths = self._bounded_paths(
                sink_name,
                effective_sources,
                adjacency,
            ...
            for path in paths[:2]:
                path.reverse()
                candidate = self._candidate(manifest, nodes, path, spec)
                candidate["search_direction"] = "reverse"
                candidates.append(candidate)
                if len(candidates) >= 12:
                    return candidates
        return candidates
    ...
    def _bounded_paths(
        ...
        *,
        ...
        expansions = 0
        while queue and len(found) < 6 and expansions < 50_000:
            path = queue.popleft()
            current = path[-1]
            hops = len(path) - 1
            if current in sinks:
    ...
    def _candidate(
        ...
        elif spec.chain_kind == "service_manager_identity_bypass":
            if "binder_caller_identity_guard" not in guard_markers:
                inferred_risks.append("calling_uid_binding_not_observed_in_bounded_path")
            if "binder_entrypoint" in source_markers and "service_manager_access" in source_markers:
                inferred_risks.append("multi_service_no_auth_pattern_observed")
