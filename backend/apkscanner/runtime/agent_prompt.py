    ...
    *,
    ...
    phase_instruction = {
        ...
        "exploration_round": (
            ...
            "recorded gap. There is no fixed tool-call or per-round test quota, but the platform "
            "does stop exploration after the configured number of consecutive rounds produces no "
            "new Evidence, changed proof state, materially changed test, or hypothesis update. "
            "Continue only while another turn can materially test, repair, falsify, or refine a "
            "security hypothesis."
        ),
        "adversarial_review": (
    ...
    return (
        ...
        "entry-point IDs. The platform has no fixed tool-call or per-round test count, but repeated "
        "rounds without new evidence or a changed proof strategy are terminated. Link each requested test to one of "
        ...
        "When platform_context.proof_replay.available=true, use the live command for a single "
        "ordinary-app replay. requested_tests remains available only when the platform must build "
        "a Harness or execute a stateful multi-step experiment; never submit the same action through "
        "both paths. "
        ...
        "When one action cannot express the required login state, callback, file transition, plugin "
        ...
        "Use cleanup_steps "
        "only for temporary test state, never target app data. A paused Capsule is inconclusive and "
        "resumable, not a negative or positive verdict. "
