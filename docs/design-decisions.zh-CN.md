...
设备池以短动态批次租用 serial：健康检查、安装/复用、动作、观察、Oracle 和清理期间不会被其他任务
穿插；Agent 调查、Critic/Rescue 和 PoC 构建不占设备。后续批次优先回到原 serial，必要迁移时重新
prepare。运行中接入新设备会扩大容量，排空只阻止新 lease，不打断当前批次。无设备时仍允许静态 lane
前进。
...
项目不设置这些固定单轮扫描配额，而使用任务生命周期、取消、设备租约、进程隔离、无事件超时和
“连续轮次没有新增 Evidence/Proof/Hypothesis/实质测试”的进展收敛保障运行。
