...
正常分析器不设置模型 step/provider-request 上限；平台仍保留可配置的任务生命周期、扫描截止时间和兼容测试轮次，用于异常恢复与资源治理。
...
8. **仍存在生命周期配置。** Worker 正常工具探索没有 step/request 上限，但任务和整单仍有可配置截止时间；上线前需要根据真实样本确定合理策略。
...
Critic 用来检查模型是否过快把静态迹象当成漏洞。它只有一次机会、最多两个异议，而且不能推翻平台 Proof，因此不会形成无止境辩论。
...
可以。配置三个 serial 后，并发数为 3；每个任务独占一台设备直至清理结束。当前不支持运行中热插拔自动扩容。
